...
def generate_pairwise_graphs(df: pd.DataFrame, col1_name: str, col2_name: str) -> Dict[str, str]:
    ...
    if col1_data.empty or col2_data.empty:
        ...
        graph_interpretations['message'] = f"No pairwise graphs generated as either '{col1_name}' or '{col2_name}' contains no data."
    ...
    if pd.api.types.is_numeric_dtype(col1_data) and pd.api.types.is_numeric_dtype(col2_data):
        try:
            ...
            ax.set_title(f'Scatter plot of {col1_name} vs {col2_name}')
            graphs['scatter_plot'] = fig_to_base64(fig)
            graph_interpretations['scatter_plot'] = f"Scatter plot of '{col1_name}' vs '{col2_name}' shows the relationship between two numerical variables. Patterns can indicate correlation."
        except Exception as e:
            graphs['scatter_plot'] = "Error generating scatter plot"
            graph_interpretations['scatter_plot'] = f"Error generating scatter plot: {e}"
        ...
        try:
            ...
            ax.set_title(f'Box plot of {col1_name} by {col2_name}')
            graphs['box_plot'] = fig_to_base64(fig)
            graph_interpretations['box_plot'] = f"Box plot of '{col1_name}' by '{col2_name}' shows the distribution of the numerical variable '{col1_name}' for each category in '{col2_name}'. Useful for comparing distributions across categories."
        except Exception as e:
            graphs['box_plot'] = "Error generating box plot"
            graph_interpretations['box_plot'] = f"Error generating box plot: {e}"

    elif (pd.api.types.is_categorical_dtype(col1_data) or col1_data.dtype == 'object') and pd.api.types.is_numeric_dtype(col2_data):
        try:
            fig, ax = plt.subplots(figsize=(10, 6))
            sns.boxplot(x=col1_data, y=col2_data, ax=ax)
            ax.set_xlabel(col1_name)
            ax.set_ylabel(col2_name)
            ax.set_title(f'Box plot of {col2_name} by {col1_name}')
            graphs['box_plot'] = fig_to_base64(fig)
            graph_interpretations['box_plot'] = f"Box plot of '{col2_name}' by '{col1_name}' shows the distribution of the numerical variable '{col2_name}' for each category in '{col1_name}'. Useful for comparing distributions across categories."
        except Exception as e:
            graphs['box_plot'] = "Error generating box plot"
            graph_interpretations['box_plot'] = f"Error generating box plot: {e}"

    elif (pd.api.types.is_categorical_dtype(col1_data) or col1_data.dtype == 'object') and (pd.api.types.is_categorical_dtype(col2_data) or col2_data.dtype == 'object'):
        try:
            fig, ax = plt.subplots(figsize=(10, 6))
            pd.crosstab(col1_data, col2_data).plot(kind='bar', stacked=True, ax=ax)
            ax.set_xlabel(col1_name)
            ax.set_ylabel('Count')
            ax.set_title(f'Stacked Bar Chart of {col1_name} vs {col2_name}')
            graphs['stacked_bar_chart'] = fig_to_base64(fig)
            graph_interpretations['stacked_bar_chart'] = f"Stacked bar chart of '{col1_name}' vs '{col2_name}' shows the distribution of categories in '{col2_name}' for each category in '{col1_name}'. Useful for visualizing the relationship between two categorical variables."
        except Exception as e:
            graphs['stacked_bar_chart'] = "Error generating stacked bar chart"
            graph_interpretations['stacked_bar_chart'] = f"Error generating stacked bar chart: {e}"
...
#         report = {
#             "headers": headers, # Include headers in the report
#             "overview": {},
#             "basic_stats": {},
#             "graphs": {},
#             "correlations": {},
...
#         # Process each column for individual analysis and basic graphs
...
#         # Generate pairwise graphs for each column against every other column
#         report["pairwise_graphs"] = {}
#         report["pairwise_graphs_interpretations"] = {}
#         for i in range(len(headers)):
#             col1 = headers[i]
#             report["pairwise_graphs"][col1] = {}
#             report["pairwise_graphs_interpretations"][col1] = {}
#             for j in range(len(headers)):
#                 if i != j: # Avoid plotting column against itself
#                     col2 = headers[j]
#                     try:
#                         print(f"DEBUG: Generating pairwise graphs for '{col1}' vs '{col2}'")
#                         pairwise_graph_results = generate_pairwise_graphs(df, col1, col2)
#                         report["pairwise_graphs"][col1][col2] = pairwise_graph_results['graphs']
#                         report["pairwise_graphs_interpretations"][col1][col2] = pairwise_graph_results['interpretations']
#                         print(f"DEBUG: Finished pairwise graphs for '{col1}' vs '{col2}'")
#                     except Exception as e:
#                         print(f"DEBUG: Error generating pairwise graphs for '{col1}' vs '{col2}': {e}")
#                         report["pairwise_graphs"][col1][col2] = {"error": "Error generating pairwise graphs"}
#                         report["pairwise_graphs_interpretations"][col1][col2] = {"error": f"Error generating pairwise graph interpretations: {str(e)}"}
...
# Helper functions (rest of the helper functions remain the same as in the previous code)
# ... (fig_to_base64, check_normality, convert_numpy_types, analyze_column, calculate_stats,
#      generate_graphs, calculate_correlations, safe_to_datetime, generate_pairwise_graphs)

@app.post("/headers")
async def get_csv_headers(file: UploadFile):
    try:
        df = pd.read_csv(file.file, parse_dates=True)
...
@app.post("/analyze")
async def analyze_data_endpoint(file: UploadFile, headers_to_process: List[str] = Form(None)): # Expect a list of headers
    try:
        df = pd.read_csv(file.file, parse_dates=True)

        if headers_to_process: # Process only selected headers
            df_processed = df[headers_to_process] # Select only the columns to process
        else:
            df_processed = df # Process all if no selection

        # Automatic datetime detection
        df_processed = df_processed.apply(lambda col: safe_to_datetime(col) if col.dtype == 'object' else col)

        report = {
            "headers": df_processed.columns.tolist(), # Headers of processed data
            "overview": {},
            "basic_stats": {},
            "graphs": {},
            "correlations": {},
        ...
        # Process each column for individual analysis and basic graphs
        for col in df_processed.columns: # Iterate over processed columns
            try:
                print(f"DEBUG: Processing column '{col}'")
        ...
        # Calculate correlations - using processed dataframe
        try:
            print("DEBUG: Calculating correlations")
        ...
        # Generate pairwise graphs - using processed dataframe
        report["pairwise_graphs"] = {}
        report["pairwise_graphs_interpretations"] = {}
        processed_headers = df_processed.columns.tolist() # Get headers of processed dataframe
        for i in range(len(processed_headers)):
            col1 = processed_headers[i]
            report["pairwise_graphs"][col1] = {}
            report["pairwise_graphs_interpretations"][col1] = {}
            for j in range(len(processed_headers)):
                if i != j:
                    col2 = processed_headers[j]
                    try:
                        print(f"DEBUG: Generating pairwise graphs for '{col1}' vs '{col2}'")
                        pairwise_graph_results = generate_pairwise_graphs(df_processed, col1, col2)
                        report["pairwise_graphs"][col1][col2] = pairwise_graph_results['graphs']
                        report["pairwise_graphs_interpretations"][col1][col2] = pairwise_graph_results['interpretations']
                        print(f"DEBUG: Finished pairwise graphs for '{col1}' vs '{col2}'")
                    except Exception as e:
